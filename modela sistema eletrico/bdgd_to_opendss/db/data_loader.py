        ...
        query = """
              WITH condutores_unicos AS (
                SELECT 
                    ...
                    sg.cnom, 
                    sg.cmax_renamed,
                    ROW_NUMBER() OVER (PARTITION BY s.tip_cnd ORDER BY ct.nome) AS rn
                FROM ssdbt s
                JOIN ctmt ct ON ct.cod_id = s.ctmt
                LEFT JOIN segcon sg ON sg.cod_id = s.tip_cnd
                WHERE s.tip_cnd IS NOT NULL
            )
            SELECT 
                nome, tip_cnd, fas_con, r1, x1, cnom, cmax_renamed
            FROM condutores_unicos
            WHERE rn = 1
            ORDER BY nome;
        ...
        query = """
           WITH condutores_unicos AS (
                SELECT 
                    ...
                    sg.cnom, 
                    sg.cmax_renamed,
                    ROW_NUMBER() OVER (PARTITION BY r.tip_cnd ORDER BY ct.nome) AS rn
                FROM ramlig r
                JOIN ctmt ct ON ct.cod_id = r.ctmt
                LEFT JOIN segcon sg ON sg.cod_id = r.tip_cnd
                WHERE r.tip_cnd IS NOT NULL
            )
            SELECT 
                nome, tip_cnd, r1, x1, cnom, cmax_renamed, fas_con
            FROM condutores_unicos
            WHERE rn = 1
            ORDER BY nome;
        """
