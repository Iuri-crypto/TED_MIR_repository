    ...
    def to_dss(df: pd.DataFrame, chunk_size=1000) -> dict:
        ...
        def to_dss_vetorizado_chunk(chunk: pd.DataFrame) -> dict:
            ...
            linhas = (
                ...
                + 'Linecode=' + chunk['tip_cnd'].astype(str) + '_linecode_baixa '
...
class LoadLowVoltage:
    ...
    def to_dss(df: pd.DataFrame, chunk_size=1000) -> dict:
        ...
        def to_dss_vetorizado_chunk(chunk: pd.DataFrame) -> dict:
            ...
            if '.4' not in chunk["rec_fases"]:
                rec = chunk["rec_fases"] + '.4'
            ...
            linhas = (
                ...
                + '_carga_baixa '
                + 'Bus1=' + chunk["pac"].astype(str) + rec.astype(str) + ' '
                + 'Phases=' + chunk["phases"].astype(str) + '\n'
        ...
        with Progress(
            ...
            for _, row in df.iterrows():
                ...
                linha = (
                    ...
                    f"New pvsystem.pv_{cod_id} phases = {phases} conn = wye bus1 = {pac}{rec_fases}\n"
                    ...
                    f"New load.{cod_id}_carga_no_pv bus1 = {pac}{rec_fases} phases = {phases}\n"
    ...
    def to_dss(df: pd.DataFrame, chunk_size=1000) -> dict:
        ...
        with Progress(
            ...
            for start in range(0, len(df), chunk_size):
                ...
                # Geração das strings
                chunk["linha"] = (
                    "New Load.nome_" + chunk["cod_id"] + "_carga_pip Bus1 = " + chunk["pac"] + rec +
                    " Phases = " + chunk["phases"].astype(str) + "\n"
                    "~ Conn = wye Model = 1 Kv = " + chunk["ten_nom_voltage"].round(5).astype(str) +
                    " Kw = " + chunk["pot_lamp"].round(5).astype(str) + " fp = 0.92\n\n"
                )
    ...
    def to_dss(df: pd.DataFrame, chunk_size=1000) -> dict:
        ...
        with Progress(
            ...
            for start in range(0, len(df), chunk_size):
                ...
                chunk.loc[mask_monobifasico, "rec_fases_t"] = chunk.loc[mask_monobifasico, "rec_fases_s"].apply(
                    lambda x: '.'.join(x.lstrip('.').split('.')[::-1]) if isinstance(x, str) else x
                )
